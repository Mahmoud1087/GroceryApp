...
class MyCartAdapter(
    ...
    override fun onBindViewHolder(holder: MyCartViewHolder, position: Int) {
        ...
        holder.txtQuantity!!.text = StringBuilder("").append(cartModelList[position].quantityOfBuy)
    ...
    private fun plusCartItem(holder: MyCartAdapter.MyCartViewHolder, cartModel: CartModel) {
        ...
        cartModel.totalPrice = cartModel.quantityOfBuy * cartModel.price!!.toFloat()
        holder.txtQuantity!!.text = StringBuilder("").append(cartModel.quantityOfBuy)
        updateFirebase(cartModel)
    }

    private fun minusCartItem(holder: MyCartAdapter.MyCartViewHolder, cartModel: CartModel) {
        if (cartModel.quantityOfBuy > 1){
            cartModel.quantityOfBuy -= 1
            cartModel.totalPrice = cartModel.quantityOfBuy * cartModel.price!!.toFloat()
            holder.txtQuantity!!.text = StringBuilder("").append(cartModel.quantityOfBuy)
            updateFirebase(cartModel)
